(function () {
    ...
    async function loadMjesta() {
        state.mjesta = await AppApi.request("/api/mjesta");
    ...
    async function loadCentri() {
        ...
        state.centri = await AppApi.request(`/api/sportski-centri${query}`);
        renderCentri();
    ...
    async function saveCentar(event) {
        ...
        const path = state.editingId
            ? `/api/sportski-centri/${encodeURIComponent(state.editingId)}`
            : "/api/sportski-centri";
        ...
        try {
            ...
            AppApi.showMessage(elements.pageMessage, "Sportski centar je spremljen.", "success");
            resetForm();
            await loadCentri();
