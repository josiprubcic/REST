(function () {
    ...
    async function init() {
        ...
        if (!state.centarId) {
            AppApi.showMessage(elements.pageMessage, "Nedostaje id sportskog centra u URL-u.", "error");
            disableForms();
            return;
        }
    ...
    function bindElements() {
        ...
        elements.radnoVrijemeTjedanOd = document.getElementById("radnoVrijemeTjedanOd");
        elements.radnoVrijemeTjedanDo = document.getElementById("radnoVrijemeTjedanDo");
        elements.radnoVrijemeVikendOd = document.getElementById("radnoVrijemeVikendOd");
        elements.radnoVrijemeVikendDo = document.getElementById("radnoVrijemeVikendDo");
        ...
        elements.resetTerenFormButton = document.getElementById("resetTerenFormButton");
    ...
    async function loadLookups() {
        const [mjesta, sportovi] = await Promise.all([
            AppApi.request("/api/mjesta"),
            AppApi.request("/api/sportovi")
        ]);
    ...
    async function loadCentar() {
        state.centar = await AppApi.request(`/api/sportski-centri/${encodeURIComponent(state.centarId)}`);
    ...
    async function loadTereni() {
        ...
        state.tereni = await AppApi.request(`/api/sportski-centri/${encodeURIComponent(state.centarId)}/tereni${query}`);
        renderTereni();
    ...
    async function saveCentar(event) {
        ...
        try {
            const saved = await AppApi.request(`/api/sportski-centri/${encodeURIComponent(state.centarId)}`, {
                method: "PUT",
                body: JSON.stringify(buildCentarPayload())
            });
            state.centar = saved;
            fillCentarForm(saved);
            AppApi.showMessage(elements.pageMessage, "Podaci sportskog centra su spremljeni.", "success");
        } catch (error) {
            AppApi.showMessage(elements.pageMessage, error.message, "error");
        }
    }
    ...
    async function deleteCentar() {
        ...
        try {
            await AppApi.request(`/api/sportski-centri/${encodeURIComponent(state.centarId)}`, {
                method: "DELETE"
            });
    ...
    async function saveTeren(event) {
        ...
        const path = isEdit
            ? `/api/sportski-centri/${encodeURIComponent(state.centarId)}/tereni/${encodeURIComponent(state.editingTerenId)}`
            : `/api/sportski-centri/${encodeURIComponent(state.centarId)}/tereni`;
        const payload = isEdit ? buildTerenUpdatePayload() : buildTerenCreatePayload();
        ...
        try {
            ...
            AppApi.showMessage(elements.pageMessage, "Teren je spremljen.", "success");
            resetTerenForm();
            await loadTereni();
        } catch (error) {
            AppApi.showMessage(elements.pageMessage, error.message, "error");
    ...
    async function deleteTeren(teren) {
        ...
        try {
            await AppApi.request(`/api/sportski-centri/${encodeURIComponent(state.centarId)}/tereni/${encodeURIComponent(teren.idTeren)}`, {
                method: "DELETE"
            });
    ...
    function fillCentarForm(centar) {
        ...
        elements.radnoVrijemeTjedanOd.value = AppApi.formatTime(centar.radnoVrijemeTjedanOd);
        elements.radnoVrijemeTjedanDo.value = AppApi.formatTime(centar.radnoVrijemeTjedanDo);
        elements.radnoVrijemeVikendOd.value = AppApi.formatTime(centar.radnoVrijemeVikendOd);
        elements.radnoVrijemeVikendDo.value = AppApi.formatTime(centar.radnoVrijemeVikendDo);
    }
